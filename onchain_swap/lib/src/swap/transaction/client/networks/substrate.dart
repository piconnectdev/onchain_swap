...
class SubstrateClient extends NetworkClient<SwapSubstrateNetwork,
    SubstrateProvider, SubstrateAddress> {
  static Future<SubstrateClient> check({
    ...
    final client =
        SubstrateClient(provider: provider, network: network, api: metadata);
  ...
  const SubstrateClient(
      {required super.provider, required super.network, required this.api});
  final MetadataApi api;

  @override
  Future<BigInt> getBalance(SubstrateAddress address) async {
    final storage =
